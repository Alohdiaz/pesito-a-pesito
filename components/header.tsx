...
export function Header({ isSidebarOpen, setIsSidebarOpen }: HeaderProps) {
  ...
  // Si el usuario ha iniciado sesión y Clerk está listo, obtiene sus datos y su suscripción
  useEffect(() => {
  ...
  return (
    <header className="fixed top-0 z-50 flex items-center justify-between w-full h-16 px-3 sm:px-4 border-b shrink-0 bg-gradient-to-b from-background/10 via-background/50 to-background/80 backdrop-blur-xl">
      <div className="flex items-center space-x-2">
        {/* Botón para abrir/cerrar el sidebar (solo si es premium) */}
        ...
        {/* Logo de la aplicación con tema dinámico claro/oscuro */}
        <Link href="/new" className="flex items-center">
        <Image
          src={currentTheme === 'dark' ? '/logo_white.png' : '/logo.png'}
          alt="Pesito a Pesito Logo"
          width={90}
          height={27}
          className="w-[75px] h-auto sm:w-[90px] transition-all"
          priority
        />
        </Link>
      ...
      <div className="flex items-center">
        <div className="hidden sm:flex items-center space-x-1 md:space-x-2">
          ...
          {/* Botón para ver precios o mostrar estado premium */}
          <Link
            href="/pricing"
            className={cn(
              buttonVariants({ variant: 'ghost', size: 'sm' }),
        ...
        {/* Botón de usuario (clerk) visible también en móvil */}
        {isSignedIn && isLoaded && (
